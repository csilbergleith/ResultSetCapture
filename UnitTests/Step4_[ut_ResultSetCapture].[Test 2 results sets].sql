...
if object_id('[ut_ResultSetCapture].[Test 2 result sets]') is not null
	drop PROCEDURE [ut_ResultSetCapture].[Test 2 result sets]
GO

CREATE procedure [ut_ResultSetCapture].[Test 2 result sets]
as
BEGIN
...
---------------------------------------------------------------
	...
	EXEC exResultSetCapture 
		  @Command='exec [dbo].[customerBalanceByLastName]	@lastName=''Brown''' -- select * from #dataTypeTable 
	...
	EXEC tSQLt.AssertEqualsTable @Expected='#expectedSummary', @Actual = '#summary';
